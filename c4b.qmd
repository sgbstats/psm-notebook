...
source("R\\prediction\\decompose_model_non_parametric.R")
...
train=data %>% filter(trt_dur>0.25)%>% filter(train) %>%  mutate(rate=exac_modsev_n/trt_dur)
test=data %>% filter(trt_dur>0.25) %>% filter(!train) %>%  mutate(rate=exac_modsev_n/trt_dur)
...
```{r, file="R\\prediction\\decompose_model.R", echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
```
...
```{r, file="R\\prediction\\c4b.R", echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
```

```{r,echo=TRUE, message=FALSE, warning=FALSE}
c4b(model, rate, arm_ipd,data=train, method= "parametric", "decompose")
```
...
c4b(model, rate, arm_ipd,data=test, method= "parametric", "decompose")
```

### Non-parametric approach

```{r, file="R\\prediction\\decompose_model_non_parametric.R", echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
```
